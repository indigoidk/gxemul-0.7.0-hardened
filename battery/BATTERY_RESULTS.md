...
4. **Honest limit (critical):** this battery validates the **happy path**. The guest→host bounds-check fixes (#99/#101/#102/#103/#113/#114 …) only trigger on *crafted/untrusted* input a normal boot never sends — so the battery does **not** prove those guards are effective; that needs a deterministic negative/reachability harness (deliberately deprioritized).
