...
for spec in $MUTANTS; do
    ...
    python3 - "$T/src/cpus/cpu_mips_coproc.c" "$const" <<'PY' || fail_setup "anchor drifted"
import io, sys
p, const = sys.argv[1], sys.argv[2]
...
PY
    ...
    ( cd "$T" && rm -f src/cpus/cpu_mips_coproc.o gxemul \
        && make -j"$(nproc)" > /tmp/m295build_$op.log 2>&1 )
    [ -x "$T/gxemul" ] || fail_setup "mutant $op did not build (see /tmp/m295build_$op.log)"

    run_probe "$T/gxemul" "/tmp/m295mut_$op.log" \
    ...
    for pair in $flips; do
        n=${pair%%=*}; want=${pair##*=}
        expect "/tmp/m295mut_$op.log" \
            "M295_ROW=pmax:$n RESULT=FAIL got=$want" || bad=1
    done
    for n in r25rp r35rm r27rm rn25rm rbnd c21rm cn225rm f29rp fn225rp \
             cvt27rm cfc1 t35rp tn35rm nan; do
        case " $flips " in *" $n="*) continue ;; esac
        expect "/tmp/m295mut_$op.log" "M295_ROW=pmax:$n RESULT=PASS" || bad=1
    done
    if [ "$bad" = 0 ]; then
