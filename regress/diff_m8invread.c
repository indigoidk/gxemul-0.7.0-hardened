 ...
 *
 *  A DISCLOSED SURVIVOR, because an honest gap beats a silent one: gating the purge on
 *  `idata != regs[...]` (mirroring the existing BWP arm) passes every row here.  It is not
 *  classified as a defect -- an unchanged value cannot change a translation -- but no
 *  count- or argument-based row can distinguish it, and saying so is the point.  Note the
 *  difference from mutant G, which IS a defect: G drops a write of zero to a register
 *  holding something else, and that is a state change being lost.
 */
#include <inttypes.h>
#include <stdarg.h>
#include <stdbool.h>
...
int main(void)
{
	...
	/*  THE CONTROL that stops this being one-sided: the WRITE side is NOT under test.
	    THESE PIN TODAY'S WRITE SIDE, NOT AN INVARIANT.  The filed `m8sarpurge` will
	    deliberately change it -- 98% of these purges are CMMU_SAR, which the translation
	    path never consults -- and when it lands E3/E4/F1 go red and must be RE-PINNED,
	    not deleted.  "CONTROL" here means "not the thing under test in THIS round".  */
	perreg_bad = 0;
	for (i = 0; i < NGROUP; i++) {
		fresh();
	...
	check_u("E3 CONTROL: writes still purge, summed", write_purges, NGROUP);
	check_u("E4 CONTROL: exactly one purge per register written", perreg_bad, 0);
	...
	for (i = 0; i < NGROUP; i++) {
		...
		if (inv_calls != 1) perreg_bad++;
		if (acc(cpu0, GROUP[i].reg * 4, MEM_READ, 0) != 0) perreg_bad++;
	}
	check_u("E9 a nonzero->ZERO write still purges once and still stores",
	    perreg_bad, 0);
	...
	/*
	 *  F1 IS THE ONLY ROW THAT CATCHES MUTANT A, and a measuring seat proved a full rig
	 *  boot does not: INVALIDATE_ALL -> INVALIDATE_VADDR leaves the count identical,
	 *  invalidates virtual page 0 alone, and still reaches `login:` 1:1:1.
	 */
	fresh();
	for (i = 0; i < NGROUP; i++)
		acc(cpu0, GROUP[i].reg * 4, MEM_WRITE, 0x2000 + i);
	check_u("F1 every write purge passes INVALIDATE_ALL (mutant A)",
	    (uint64_t) (inv_all_were_ALL && inv_calls == NGROUP), 1);
	...
	check("IDENTITY row count -- guards against a stale copy", buf, "21");
