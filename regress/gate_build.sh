...
DIVERGENT="devices/dev_jazz.c      #257 arc-only interrupt gating; CHANGELOG.md:653 records it as not affecting pmax
devices/Makefile.skel   mechanically tied to the SEC-only dev_ne2000.o
disk/diskimage.c        SUSPECT: the whole diff is trailing whitespace on one blank line, from root import 39748e3. The #115 fix beneath it is byte-identical in both trees. Normalise and drop this entry.
machines/machine_arc.c  every hunk gated on ARC/PICA conditionals; foundation commit 9d18d15
promemul/arcbios.c      every hunk gated on MACHINE_ARC; 9d18d15 plus later hardening
devices/dev_ne2000.c    SEC-exclusive: the file does not exist in est at all
devices/autodev.c       GENERATED by enumerating dev_*.c (makeautodev.sh:56-64,81-90 globs the directory and greps DEVINIT). dev_ne2000.c is the only device file SEC has and est lacks, so this diff is exactly its declaration plus its device_register() call. It can NEVER be identical while the device sets differ."
