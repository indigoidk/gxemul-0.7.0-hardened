...
| `130ff1b` | Round 89 (#330): #327 made FPSCR's VX and FEX derived and correct — this measured what they derived *over*, and it was nothing. **Ten exception causes, none ever raised**: no VXISI for Inf−Inf, no VXZDZ for 0/0, no VXIMZ for Inf×0, no ZX for a divide by zero, not even VXSNAN for a signalling operand. Seventeen rows pre-registered from Book I by two seats independently, all matching first run — including seven negative controls, without which a handler that raised indiscriminately would pass every positive row |
| `85bc467` | Round 90 (#329): three more Thumb flag defects, found by the previous round's completeness sweep — **ASR-immediate set Z and N from a register selected by the shift amount** (`asrs r0,r1,#12` took its flags from r3), `RORS` never cleared Z/N so stale flags survived, and `LSR #0`/`ASR #0` executed as shift-by-zero where the encoding means shift-by-32. Half the contract was unmeasurable until the Thumb rig learned to preset carry |
| `d6f3d75` | Round 77 (#328): the Thumb interpreter got **three** of its four condition flags wrong, in four near-identical blocks — Z read off an untruncated 64-bit value (so `subs; bne` never terminated), C derived from the carry-out of a negation (so subtracting zero reported a borrow), and V from the sign of the *negated* subtrahend, which breaks at `0x80000000`, the one value that is its own negation. Only N, which truncated first, was right |
| `ad0e67d` | Round 88 (#327): FPSCR's VX and FEX were stored where the ISA defines them as derived, so they went stale both ways — a guest could clear the last invalid-operation cause and be left with **VX set and nothing causing it**, a state hardware cannot produce, which #326 is what made reachable. `mtfsf` stopped copying them in the same commit, because its unmasked write was the only escape from that phantom. Twelve gate expectations were registered from Book I before the code was written and all twelve matched first run |
...
`regress/run.sh` runs fourteen gates; `regress/run.sh 2 4` runs a subset. The governing rule is
